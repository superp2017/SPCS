...
void HandleControl::init_connect()
{
    ...
    connect(this,SIGNAL(close_allDEC()),this,SLOT(on_pushButton_closeall_clicked()),
            Qt::QueuedConnection);
    connect(ui->checkBox_enable_sweep,SIGNAL(clicked(bool)),ui->lineEdit_sweeptime,SLOT(setEnabled(bool)));
...
void HandleControl::init_data()
{
    ...
    ui->lineEdit_sweeptime->setValidator(new QDoubleValidator(0.0, 100000000000.0,5,this));
    default_wave();
    default_sweepshelf();
...
void HandleControl::on_pushButton_default_vna_clicked()
{
    ...
    ui->lineEdit_sweeptime->setText("20");
    ...
    ui->checkBox_enable_sweep->setChecked(false);
    ...
    ui->lineEdit_sweeptime->setEnabled(false);
    ui->checkBox_enable_sweep->setChecked(false);
...
void HandleControl::on_pushButtonapply_clicked()
{
    ...
    }else
    {
        ...
        if(VNAdapter::instance()->connect())
        {
            ...
            if(ui->checkBox_enable_sweep->isChecked())
                VNAdapter::instance()->set_sweep_time(ui->lineEdit_sweeptime->text().toDouble());
...
void HandleControl::on_pushButton_stable_clicked()
{
    ...
    m_0_path = ui->lineEdit_savepath->text() + QString("/")+ \
            cur_time+QString("_")+QString("dBm") + \
...
void HandleControl::on_pushButton_collect_clicked()
{
    ...
    m_collect_path = ui->lineEdit_savepath->text() \
            +QString("/")+ QDateTime::currentDateTime().toString("yyyy_MM_dd_hh")+\
