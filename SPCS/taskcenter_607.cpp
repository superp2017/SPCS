...
////转化最终的校准数据,主要是把幅度以0.5做不进,相位以0.625做步进
...
#if  1
                                double old_rel  = m_cal_temp_s22_status_1[dec][cur_freq].first\
                                        -m_cal_temp_s22_status_2[dec][cur_freq].first;
                                double old_img = m_cal_temp_s22_status_1[dec][cur_freq].second\
                                        - m_cal_temp_s22_status_2[dec][cur_freq].second;

                                double new_rel  = m_cal_new_s22_status_1[pos][dec][cur_freq].first\
                                        -m_cal_new_s22_status_2[pos][dec][cur_freq].first;
                                double new_img = m_cal_new_s22_status_1[pos][dec][cur_freq].second \
                                        - m_cal_new_s22_status_2[pos][dec][cur_freq].second;
#endif
                                double delt_rel = new_rel  - old_rel;
                                double delt_img = new_img  - old_img;

                                double delt_magn  = VNAdapter::instance()->genLinMag(delt_rel,delt_img);
                                double delt_phase = VNAdapter::instance()->genPhase(delt_rel,delt_img)/2;
...
bool TaskCenter_607::ROW_TEST(TDATA &Pdata, Task_Cmp *pCalData)
{
    ...
    for(size_t i=0; i<m_P.size(); i++)
    {
        ...
        for(size_t j=0; j<ls_D.size(); j++)
        {
            ...
            int single_dec = decnum;
            ///////////////////测试单个或多个探头位置////////////////////////////////
            if(ENV_config::instance()->EnableSingleDectorTest())
            {
                single_dec = ENV_config::instance()->SingleDector();
            }
            ////////////////////////////////////////////////////////
            ...
            TSWMatrixs::instance()->turn_on(CVTS::Matrix(single_dec-1));
            BuziWidget::instance()->muti_switch_s(single_dec-1);
            sprintf(buf,"切换探头至%d",single_dec);
            OutputView::instance()->append(buf);

            ::Sleep(m_elesw_time);

            BuziWidget::instance()->switch_off_s(16);
            BuziWidget::instance()->switch_off_s(single_dec-1);
            BuziWidget::instance()->detector_off_s(single_dec-1);
            ...
            for(size_t uid =0;uid<cur_id_vec.size();++uid)
            {
                ...
                /////显示监控界面上对应的测试阵元
                emit SHOWUNIT(id);
                if(!pCalData)///////只有ORG测试且没有选择初始配相表才会成立
                {
                    ///打开波控
                    if(initWaveUnit(id))
                    {
                    ...
                    /* QString::number((rand()%200)/190,'f',3)+QString(",")+QString::number((rand()%190)/180,'f',3);*/
                ...
                }else
                {
                    ...
                    for(int f = 0;f<freq_size;++f)
                    {
                        ...
                        /// S21
                        /*QString::number((rand()%200)/190,'f',3)\
            ...
            if(pCalData)
            {
                VNAdapter::instance()->set_freq(\
                            m_freqs.at(0).toDouble(),\
            ...
            if(m_is_real_Cal)
            {
                if(m_is_collect_s11)
                {
                    ...
                    //QString::number((rand()%200)/190,'f',3)\
                    // +QString(",")+QString::number((rand()%190)/180,'f',3);
                    QString s11 = VNAdapter::instance()->collect_S11();
                    ...
                    if(list_s11.size() == freq_size*2&& list_s11.size() >1 )
                    {
                        for(int l = 0; l <freq_size;++l)
                        {
                            ...
                            for(size_t uid =0;uid<cur_id_vec.size();++uid)
                            {
                                int id = cur_id_vec.at(uid);
                                CHECK_STOP;
                                Pdata.t_task_s11_close[cur_freq][id] =  str_s11;
                            }
                        }
                    }
                }
                ...
                ///采集 S22//////电子开关关闭状态
                QString ps22 = VNAdapter::instance()->collect_S22();
                ...
                if(plist_s22.size() == freq_size*2 )
                {
                    for(int l = 0; l <freq_size;++l)
                    {
                        ...
                        double  pampl   =  real_22.toDouble();
                        double  pphase  =  imag_22.toDouble();

                        QString str_s22 = real_22 +"," + imag_22;
                        for(size_t uid =0;uid<cur_id_vec.size();++uid)
                        {
                            int id = cur_id_vec.at(uid);
                            Pdata.t_task_s22_close[cur_freq][id] =  str_s22;
                        }
            ...
            /////////////////打开电子开关/////////////////////////
            TEleSwitch::instance()->turn_on(CVTS::CVTS_Decetor(single_dec-1));
            BuziWidget::instance()->switch_on_s(16);
            BuziWidget::instance()->switch_on_s(single_dec-1);
            BuziWidget::instance()->detector_on_s(single_dec-1);
                ...
                if(list_s11.size() == freq_size*2&& list_s11.size() >1 )
                {
                    for(int l = 0; l <freq_size;++l)
                    {
                        ...
                        for(size_t uid =0;uid<cur_id_vec.size();++uid)
                        {
                            int id = cur_id_vec.at(uid);
                            CHECK_STOP;
                            Pdata.t_task_s11_open[cur_freq][id] =  str_s11;
                        }
                    }
                }
            ...
            if(list_s22.size() == freq_size*2 && list_s22.size() >1 )
            {
                for(int l = 0; l <freq_size;++l)
                {
                    ...
                    {
                        int id = cur_id_vec.at(uid);
                        Pdata.t_task_s22_open[cur_freq][id] =  str_s22;
                    }
            ...
            //////////////////////采集 S22 状态1///////////////////////////////////////////////////
            ////QString::number((rand()%180)/190,'f',3)+QString(",")+QString::number((rand()%210)/180,'f',3);
            QString s22 =  VNAdapter::instance()->collect_S22();
            QStringList list_s22 = s22.split(QRegExp(","));
            ...
            if(m_is_real_Cal)
            {
                ...
                /////////////采集S22 状态2/////////////////////////////////////////////////////////////
                ///QString::number((rand()%210)/190,'f',3)+QString(",")+QString::number((rand()%190)/190,'f',3);
                QString ps22 =  VNAdapter::instance()->collect_S22();
                ::Sleep(m_pnacollect_time);
                QStringList plist_s22 = ps22.split(QRegExp(","));
                if(plist_s22.size() != freq_size*2)
            ...
            ///////////////////关闭当前监控界面上的的开关////////////////////////////
            TEleSwitch::instance()->turn_off(CVTS::CVTS_Decetor(single_dec-1));
            BuziWidget::instance()->switch_off_s(16);
            BuziWidget::instance()->switch_off_s(single_dec-1);
        }
    }
...
////
/// \brief TaskCenter_607::CONTROL_WAVE
/// \param p_freq
/// \param id
/// \param p_Cal_data
/// \return
...
///
/// \brief TaskCenter_607::gen_fileName
/// \param index
/// \param type
/// \param typeindex
/// \return
                                   ...
                                   ,QString psave_path)
{
    ...
    {
        m_DataManage.SaveToFile(pdata.t_task_s21,p_freqs,psave_path + "/"+ pdata.t_s21_Fname +QString("_open")+ ORG_SUFFIX);
        m_DataManage.SaveToFile_dB(pdata.t_task_s21,p_freqs,psave_path + "/" + pdata.t_s21_Fname + MP_SUFFIX);
    }


    if(!pdata.t_task_s11_open.empty())
    {
        m_DataManage.SaveToFile(pdata.t_task_s11_open,p_freqs,psave_path + "/"+ pdata.t_s11_Fname+QString("_open") + ORG_SUFFIX);
        m_DataManage.SaveToFile_dB(pdata.t_task_s11_open,p_freqs,psave_path + "/" +  pdata.t_s11_Fname+QString("_open")  + MP_SUFFIX);
    }

    if(!pdata.t_task_s11_close.empty())
    {
        m_DataManage.SaveToFile(pdata.t_task_s11_close,p_freqs,psave_path + "/"+ pdata.t_s11_Fname +QString("_close") + ORG_SUFFIX);
        m_DataManage.SaveToFile_dB(pdata.t_task_s11_close,p_freqs,psave_path + "/" +  pdata.t_s11_Fname+QString("_close")  + MP_SUFFIX);
    }

    if(!pdata.t_task_s22_open.empty())
    {
        m_DataManage.SaveToFile(pdata.t_task_s22_open,p_freqs,psave_path + "/"+ pdata.t_s22_Fname +QString("_open")+ ORG_SUFFIX);
        m_DataManage.SaveToFile_dB(pdata.t_task_s22_open,p_freqs,psave_path + "/" + pdata.t_s22_Fname + MP_SUFFIX);
    }

    if(!pdata.t_task_s22_close.empty())
    {
        m_DataManage.SaveToFile(pdata.t_task_s22_close,p_freqs,psave_path + "/"+ pdata.t_s22_Fname  +QString("_close")+ ORG_SUFFIX);
        m_DataManage.SaveToFile_dB(pdata.t_task_s22_close,p_freqs,psave_path + "/" + pdata.t_s22_Fname  +QString("_close")+ MP_SUFFIX);
    }
...
/////初态测试后，如果人为修改，弹出界面后,才会掉此函数，把修改过后的基准值返回到流程中
...
///
/// \brief TaskCenter_607::check_success
/// \param p_cal_mp_1
/// \return
/// 给一个校准数据，检验是否收敛：(幅度小于0.5的单元数)/总单元数<5% && (相位大于300°的单元数)/总单元数<5%
...
void TaskCenter_607::clear_buf()
{
    ...
    m_task_org_data.t_task_s11_open.clear();
    m_task_org_data.t_task_s21.clear();
    m_task_org_data.t_task_s22_open.clear();
