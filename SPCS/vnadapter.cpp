...
{
    connect();
    viPrintf(m_vi, ViString("SENS:AVER ON \n"));
    sprintf(m_buffer, ViString("SENS:AVER:COUN %d\n"), times);
...
{
    connect();
    sprintf(m_buffer, ViString("SENS1:SWE:TIME %f\n"), sec/1000);
    viPrintf(m_vi, m_buffer);
...
void VNAdapter::set_freq(QString &freq)
{
    ...
    /// 设置频率点
    sprintf(m_buffer, ViString("SENS1:SWE:POIN %d\n"), 1);
    viPrintf (m_vi, m_buffer);
    sprintf(m_buffer, ViString("SENS1:FREQ:STAR %fHz\n"), freq.toDouble());
    viPrintf (m_vi, m_buffer);
    sprintf(m_buffer, ViString("SENS1:FREQ:STOP %fHz\n"), freq.toDouble());
    viPrintf (m_vi, m_buffer);
...
{
    connect();
    viPrintf (m_vi, ViString("sens:path:conf:elem 'PulseModDrive','Pulse1'\n"));
    viPrintf (m_vi, ViString("sour:pow:alc:mode openloop \n"));
    viPrintf (m_vi, ViString("sens:path:conf:elem 'Src1Out1PulseModEnable','Enable'\n"));

    viPrintf (m_vi, ViString("sens:path:conf:elem 'PulseTrigInput','Internal'\n"));
    ...
    viPrintf (m_vi, ViString("SENS:PULS0:STAT 1\n"));
    viPrintf (m_vi, ViString("SENS:PULS1:STAT 1\n"));
    viPrintf (m_vi, ViString("SENS:PULS2:STAT 1\n"));
    viPrintf (m_vi, ViString("SENS:PULS3:STAT 1\n"));
    viPrintf (m_vi, ViString("SENS:PULS4:STAT 1\n"));

    viPrintf (m_vi, ViString("sens:puls:per .001\n"));

    viPrintf (m_vi, ViString("sens:puls1:width 0.0005\n"));
    viPrintf (m_vi, ViString("sens:puls1:delay 0.00008\n"));
    viPrintf (m_vi, ViString("sens:puls0:width 0.000001\n"));
    viPrintf (m_vi, ViString("sens:puls0:delay 0.0000004\n"));

    viPrintf (m_vi, ViString("sens:puls2:width 0.00025\n"));
    viPrintf (m_vi, ViString("sens:puls2:delay 0.00004\n"));
    viPrintf (m_vi, ViString("sens:puls3:width 0.00012\n"));
    viPrintf (m_vi, ViString("sens:puls3:delay 0.00002\n"));
    viPrintf (m_vi, ViString("sens:puls4:width 0.00006\n"));
    viPrintf (m_vi, ViString("sens:puls4:delay 0.00001\n"));
    ///////
    viPrintf (m_vi, ViString("SENS:SWE:GEN STEP\n"));
    viPrintf (m_vi, ViString("TRIG:SOUR MAN\n"));
                         ...
                         double path4_wi)
{
    connect();
    if(!iseable)
        viPrintf (m_vi, ViString("sens:path:conf:elem 'Src1Out1PulseModEnable','Disable'\n"));
    else
    {
        viPrintf (m_vi, ViString("sens:path:conf:elem 'PulseModDrive','Pulse1'\n"));
        viPrintf (m_vi, ViString("sour:pow:alc:mode openloop \n"));
        ...
        int ret = (path0 ? 1:0);
        sprintf(m_buffer, ViString("SENS:PULS0:STAT %d\n"), ret);
        viPrintf (m_vi, m_buffer);

        ret = (path1 ? 1:0);
        sprintf(m_buffer, ViString("SENS:PULS1:STAT %d\n"), ret);
        viPrintf (m_vi, m_buffer);

        ret = (path2 ? 1:0);
        sprintf(m_buffer, ViString("SENS:PULS2:STAT %d\n"), ret);
        viPrintf (m_vi, m_buffer);

        ret = (path3 ? 1:0);
        sprintf(m_buffer, ViString("SENS:PULS3:STAT %d\n"), ret);
        viPrintf (m_vi, m_buffer);

        ret = (path4 ? 1:0);
        sprintf(m_buffer, ViString("SENS:PULS4:STAT %d\n"), ret);
        viPrintf (m_vi, m_buffer);
        ...
        sprintf(m_buffer, ViString("sens:puls:per %.9lf\n"), ls_period);
        viPrintf (m_vi, m_buffer);

        //  sprintf(m_buffer, "sens:puls0:width %.9lf\n", ls_path0_wi);
        // viPrintf (m_vi, m_buffer);
        sprintf(m_buffer, ViString("sens:puls0:delay %.9lf\n"), ls_path0_de);
        viPrintf (m_vi, m_buffer);

        sprintf(m_buffer, ViString("sens:puls1:width %.9lf\n"), ls_path1_wi);
        viPrintf (m_vi, m_buffer);
        sprintf(m_buffer, ViString("sens:puls1:delay %.9lf\n"), ls_path1_de);
        viPrintf (m_vi, m_buffer);

        sprintf(m_buffer, ViString("sens:puls2:width %.9lf\n"), ls_path2_wi);
        viPrintf (m_vi, m_buffer);
        sprintf(m_buffer, ViString("sens:puls2:delay %.9lf\n"), ls_path2_de);
        viPrintf (m_vi, m_buffer);

        sprintf(m_buffer, ViString("sens:puls3:width %.9lf\n"), ls_path3_wi);
        viPrintf (m_vi, m_buffer);
        sprintf(m_buffer, ViString("sens:puls3:delay %.9lf\n"), ls_path3_de);
        viPrintf (m_vi, m_buffer);

        sprintf(m_buffer, ViString("sens:puls4:width %.9lf\n"), ls_path4_wi);
        viPrintf (m_vi, m_buffer);
        sprintf(m_buffer, ViString("sens:puls4:delay %.9lf\n"), ls_path4_de);
        viPrintf (m_vi, m_buffer);
...
void VNAdapter::set_trace_S11()
{
    ...
    /// 设置端口通道1 S11
    viPrintf (m_vi, ViString("CALC1:PAR 'ch1_S11',S11,1\n"));
    /// 设置显示窗口
...
void VNAdapter::set_trace_S22()
{
    connect();
    /// 设置端口通道1 S22
    viPrintf (m_vi, ViString("CALC1:PAR 'ch1_S22',S22,1\n"));
    /// 设置显示窗口
...
void VNAdapter::set_trace_S21()
{
    connect();
    /// 设置端口通道1 S21
    viPrintf (m_vi, ViString("CALC1:PAR 'ch1_S21',S21,1\n"));
    /// 设置显示窗口
...
void VNAdapter::set_trace_S12()
{
    connect();
    /// 设置端口通道1 S21
    viPrintf (m_vi, ViString("CALC1:PAR 'ch1_S12',S12,1\n"));
    /// 设置显示窗口
...
void VNAdapter::set_trace_S31()
{
    connect();
    /// 设置端口通道1 S31
    viPrintf (m_vi, ViString("CALC1:PAR 'ch1_S31',S31,1\n"));
    /// 设置显示窗口
...
void VNAdapter::set_trace_S33()
{
    connect();
    /// 设置端口通道1 S33
    viPrintf (m_vi, ViString("CALC1:PAR 'ch1_S33',S33,1\n"));
    /// 设置显示窗口
